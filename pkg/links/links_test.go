...
func Test_extract(t *testing.T) {
	...
	htmlStr := strings.NewReader(`<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
...
<a href='http://www.amazon.com/dp/020161586X?tracking_id=disfordig-20'>The Practice of Programming</a>.
	...
	u, _ := url.Parse("http://gopl.io")
	doc, _ := html.Parse(htmlStr)

	got, err := extract(u, doc)
	if err != nil || reflect.DeepEqual(got, expectLinks) {
		panic(fmt.Sprintf("Test_extract assert failed: err=%v, got=%v, expect=%v\n", err, got, expectLinks))
	}
}

func GetGolangMainPage() string {
	resp, err := http.Get("http://gopl.io")
	if err != nil || resp.StatusCode != http.StatusOK {
		panic("get http://gopl.io failed")
	}
	bytes, err := io.ReadAll(resp.Body)
	if err != nil {
		panic("get content of webpage body failed")
	}
	return string(bytes)
}
